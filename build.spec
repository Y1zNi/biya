...
if bundle_browsers.is_dir():
  ...
  raise SystemExit(
    ...
    'Run python build.py first (downloads and stages Chromium).'
  )
...
bundle_node_exe = project_root / '_bundle' / 'node' / 'node.exe'
if bundle_node_exe.is_file():
  datas.append((str(bundle_node_exe), 'node'))
else:
  raise SystemExit(
    f'Missing embedded Node: {bundle_node_exe}\n'
    'Run python build.py first (downloads and stages node.exe).'
  )
